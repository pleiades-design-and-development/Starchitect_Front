...
export default class Base_Profile extends React.Component {
    ...
    render() {
        ...
        return ( <
            ...
            id = 'rank' >
            <
            img src = 'cadetBadge.svg'
            alt = 'rank badge' / >
            <
            p style = {
                { textTransform: 'uppercase', marginTop: '2px', marginLeft: '5px' } }
            className = 'rank' > { Object.keys(user).length ? user.attributes.rank : 'Rank' } < /p> <
            /Menu.Item> <
            Menu.Item name = 'progress-bar'
            id = 'progress_bar' >
            <
            p > Experience Points < /p> <
            Progress size = 'small'
            percent = { Object.keys(user).length ? user.relationships.points.data : 0 }
            indicating / >
            <
            /Menu.Item> <
            Menu.Item id = 'mode' >
            ...
            Last Known Location:
            <
            Popup trigger = { < Icon circular name = 'question circle outline'
                className = 'location_help' / > }
            ...
            /p> <
            p > < Link to = { './Jupiter' }
            className = 'link-color' > Jupiter < /Link></p >
            <
            /Menu.Item> <
            ...
            <
            p > < Link to = { './Io' }
            className = 'link-color' > Io < /Link></p >
            <
            p > < Link to = { './Europa' }
            className = 'link-color' > Europa < /Link></p >
            <
            p > < Link to = { './Mars' }
            className = 'link-color' > Mars < /Link></p >
            <
            Link to = '/Beacons'
            className = 'link-color' > More... < /Link> <
            /Menu.Item> <
            Menu.Item name = 'feed_link' >
            <
            Link to = '/Profile'
            className = 'link-color' > News Feed < /Link> <
            /Menu.Item> <
            Menu.Item name = 'submissions' >
            <
            Link to = '/Submissions'
            className = 'link-color' > Submissions < /Link> <
            /Menu.Item> <
            Menu.Item name = 'crew-lounge' >
            <
            Link to = '/CrewLounge'
            className = 'link-color' > Crew Lounge < /Link> <
            /Menu.Item> <
            Menu.Item >
            <
